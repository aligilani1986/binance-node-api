...
let api = function Binance(options = {}) {
    ...
    let sapiTest = 'https://api.binance.com/sapi/';
    ...
    let streamTest = 'wss://testnet.binance.vision/ws/';
    let combineStream = 'wss://stream.binance.com:9443/stream?streams=';
    let combineStreamTest = 'wss://testnet.binance.vision/stream?streams=';
    ...
    function setOptions(opt = {}, callback = false) {
        ...
        if (typeof Binance.options.urls !== 'undefined') {
            ...
            if (typeof urls.baseTest === 'string') baseTest = urls.baseTest;
            ...
            if (typeof urls.streamTest === 'string') streamTest = urls.streamTest;
            if (typeof urls.combineStreamTest === 'string') combineStreamTest = urls.combineStreamTest;
        ...
        if (Binance.options.useServerTime) {
            publicRequest((Binance.options.test ? baseTest : base) + 'v3/time', {}, function (error, response) {
    ...
    const subscribe = function (endpoint, callback, reconnect = false, opened_callback = false) {
        ...
        if (socksproxy !== false) {
            ...
            ws = new WebSocket((Binance.options.test ? streamTest : stream) + endpoint, { agent: agent });
            ...
            ws = new WebSocket((Binance.options.test ? streamTest : stream) + endpoint);
        }
     ...
     */
        ...
        websockets: {
            ...
            userData: function userData(callback, execution_callback = false, subscribed_callback = false, list_status_callback = false) {
                ...
                apiRequest((Binance.options.test ? baseTest : base) + 'v3/userDataStream', {}, function (error, response) {
                    Binance.options.listenKey = response.listenKey;
                    setTimeout(function userDataKeepAlive() { // keepalive
                        try {
                            apiRequest((Binance.options.test ? baseTest : base) + 'v3/userDataStream?listenKey=' + Binance.options.listenKey, {}, function (err) {
           ...
           */
            ...
            depth: function depth(symbols, callback) {
                ...
                if (Array.isArray(symbols)) {
                    ...
                    let symbol = symbols;
                    subscription = subscribe(symbol.toLowerCase() + '@depth20@100ms', callback, reconnect);
                }
                return subscription.endpoint;
            },
